...
public class Kruskal {
	...
	public Kruskal(int n, String[] serieNodos) {
		...
		G = new int[nodos.length][nodos.length];

		this.in = new boolean[n + 1][n + 1];
		this.t = new int[n + 1][3];

		// DISTANCIA DE LAS ARISTAS
		/*
		 * System.out.print("\nDame la distancia de los caminos:\n");
		 * for(int i=1;i<=n;i++)
		 * for(int j=1;j<=n;j++)
		 * {
		 * in[i][j] = in[j][i] = false;
	...
	public void AgregarRutaKruskal(String origen, String destino, int distancia) {
